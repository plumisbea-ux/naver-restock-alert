...
function dashboardMetrics(store) {
  ...
  return {
    ...
    customerCount: Object.keys(store.customer_contexts || {}).length
  };
}
...
function adminState() {
  ...
  return {
    ...
    customer_contexts: store.customer_contexts || {}
  };
}

async function handleAdminPost(eventBody) {
  if (eventBody.event === "admin.reset") {
    resetDb();
    return { ok: true, action: "reset", state: adminState() };
  }
  ...
  if (eventBody.event === "admin.update_stock") {
    ...
    return { ok: true, action: "update_stock", result, state: adminState() };
  }
...
export default {
  async fetch(request) {
    ...
    if (request.method === "GET") {
      if (mode === "admin-state") return json(adminState());
      return json({
        ok: true,
    ...
    const adminResponse = await handleAdminPost(eventBody);
    if (adminResponse) return json(adminResponse, adminResponse.ok ? 200 : 400);
