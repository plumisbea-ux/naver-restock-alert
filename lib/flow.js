...
export function handleOpenEvent(eventBody) {
  ...
  updateSession(talkUserId, {
    stage: selectedOption?.product_id === product.id ? "CHANNEL_SELECT" : "OPENED_PRODUCT",
    product_id: product.id,
    option_id: selectedOption?.product_id === product.id ? selectedOption.id : null,
    awaiting_phone: false
  });

  if (selectedOption?.product_id === product.id) return channelSelectPrompt(product, selectedOption);
  return initialProductPrompt(product);
}
