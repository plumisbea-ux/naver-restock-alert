...
import { sendMockNaverTalk } from "./mockSenders.js";
import {
  channelSelectPrompt,
  completePrompt,
  fallbackPrompt,
...
import { detectIntent, extractOptionFromOpenEvent, extractProductFromOpenEvent } from "./naverTalkParser.js";
...
function scheduleTutorialReturnGuide(waitlist, waitingCount) {
  waitUntil(new Promise((resolve) => setTimeout(resolve, 600)).then(() => sendMockNaverTalk({
    waitlist,
    waitingCount,
    text: "↩️ 이제 데모 페이지로 돌아가주세요!",
    messageType: "TUTORIAL_RETURN_GUIDE"
  })));
}
...
function completeWaitlist({ talkUserId, productId, optionId, channelPreference, phoneNumber = null, smsOptIn = false, request, profileRequestStatus = null }) {
  ...
  scheduleTutorialReturnGuide(waitlist, waitCount);
...
export function handleOpenEvent(eventBody) {
  ...
  const existingSession = getSession(talkUserId);
  const product = extractProductFromOpenEvent(eventBody);
  const selectedOption = extractOptionFromOpenEvent(eventBody);

  if (!product) {
    // 단순 채팅방 재접속/새로고침은 기존 신청 상태를 지우거나 안내 메시지를 추가하지 않습니다.
    if (existingSession.product_id) return null;
    updateSession(talkUserId, {
      stage: "PRODUCT_SELECT",
      product_id: null,
      option_id: null,
  ...
  updateSession(talkUserId, {
    stage: selectedOption?.product_id === product.id && Number(selectedOption.stock_quantity) <= 0 ? "CHANNEL_SELECT" : "OPENED_PRODUCT",
    product_id: product.id,
    option_id: selectedOption?.product_id === product.id && Number(selectedOption.stock_quantity) <= 0 ? selectedOption.id : null,
    awaiting_phone: false
  });

  if (selectedOption?.product_id === product.id && Number(selectedOption.stock_quantity) <= 0) return channelSelectPrompt(product, selectedOption);
  return initialProductPrompt(product);
}
