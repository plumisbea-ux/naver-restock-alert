...
import { detectIntent, extractProductFromOpenEvent, extractProductNoFromOpenEvent } from "./naverTalkParser.js";

function pickPhoneForMockUser(talkUserId) {
  return db().mock_users[talkUserId]?.mock_phone_number || null;
}

function getProductOrFallback(productId, talkUserId = null) {
  if (productId) return getProductById(productId) || getDefaultProduct();
  if (talkUserId) {
    const context = getCustomerContext(talkUserId);
    if (context?.product_id) return getProductById(context.product_id) || getDefaultProduct();
  }
  return getDefaultProduct();
}

function getProductIdFromCustomerContext(talkUserId) {
  return getCustomerContext(talkUserId)?.product_id || null;
}
...
export function handleOpenEvent(eventBody) {
  const talkUserId = eventBody.user || `unknown_${Date.now()}`;
  const productNo = extractProductNoFromOpenEvent(eventBody);
  let product = extractProductFromOpenEvent(eventBody);

  if (!product) {
    const rememberedProductId = getProductIdFromCustomerContext(talkUserId);
    product = rememberedProductId ? getProductById(rememberedProductId) : null;
  }
  ...
  if (!product) {
    ...
    registerCustomerOpen({ talkUserId, productId: null, productNo, rawEvent: eventBody });
    return productContextMissingPrompt(db().products);
  }

  registerCustomerOpen({ talkUserId, productId: product.id, productNo: productNo || product.product_no, rawEvent: eventBody });
...
export function handleSendEvent(eventBody, request) {
  ...
  switch (intent.type) {
    case "APPLY_RESTOCK": {
      return startApplyFlow(talkUserId, intent.productId || session.product_id || getProductIdFromCustomerContext(talkUserId));
    }

    case "OTHER_INQUIRY": {
      const product = getProductOrFallback(intent.productId || session.product_id, talkUserId);
      updateSession(talkUserId, {
        stage: "OTHER_INQUIRY",
        product_id: product.id,
        awaiting_phone: false
    ...
    case "CHANNEL_NAVER_ONLY": {
      const productId = intent.productId || session.product_id || getProductIdFromCustomerContext(talkUserId);
    ...
    case "CHANNEL_KAKAO": {
      updateSession(talkUserId, {
        stage: "KAKAO_CONSENT",
        product_id: intent.productId || session.product_id || getProductIdFromCustomerContext(talkUserId),
