...
export function buildRestockText({ product, option, waitCount }) {
  return [
    ...
    "",
    "✅ 기다리시던 옵션이 다시 입고되었습니다.",
    "",
    `👥 현재 대기인원: ${waitCount}명`,
    "⏰ 준비된 재고가 빠르게 소진될 수 있어요.",
    `🔗 지금 상품 확인하기: ${product.product_url}`
  ].join("\n");
}
...
export async function sendMockNaverTalk({ waitlist, waitingCount = null, text = null, messageType = "RESTOCK_ALERT" }) {
  ...
  const body = {
    ...
    textContent: {
      text: text || buildRestockText({ product, option, waitCount })
    },
    options: {
      notification: true
    }
