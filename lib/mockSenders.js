...
export async function sendMockNaverTalk({ waitlist }) {
  const product = getProductById(waitlist.product_id);
  const option = getOptionById(waitlist.option_id);
  const waitCount = getWaitlistCount(waitlist.product_id, waitlist.option_id);
  const payload = {
...
export async function sendMockKakaoAlimtalk({ waitlist }) {
  const product = getProductById(waitlist.product_id);
  const option = getOptionById(waitlist.option_id);
  const waitCount = getWaitlistCount(waitlist.product_id, waitlist.option_id);
  const payload = {
