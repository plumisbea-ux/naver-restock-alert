...
const statePath = "state/naver-restock-alert.json";
const redisKey = "naver-restock-alert:state:v1";
...
export async function hydrateDb() {
  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    const response = await redisCommand(["GET", redisKey]);
    if (response?.result) globalThis[globalKey] = JSON.parse(response.result);
    return db();
  }
  if (!process.env.BLOB_READ_WRITE_TOKEN) return db();
  try {
    const result = await get(statePath, { access: "private" });
    if (result?.statusCode === 200 && result.stream) {
      const text = await new Response(result.stream).text();
      globalThis[globalKey] = JSON.parse(text);
    }
  } catch (error) {
...
export async function persistDb() {
  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    return redisCommand(["SET", redisKey, JSON.stringify(db())]);
  }
  if (!process.env.BLOB_READ_WRITE_TOKEN) return null;
  return put(statePath, JSON.stringify(db()), {
    access: "private",
...
export function upsertWaitlist({
  sellerId,
  productId,
  optionId,
  talkUserId,
  ...
  if (existing) {
    ...
    existing.sms_opt_in = Boolean(smsOptIn || existing.sms_opt_in || channelPreference === "NAVER_TALK_AND_SMS");
    existing.channel_preference = channelPreference || existing.channel_preference;
    existing.profile_request_status = profileRequestStatus ?? existing.profile_request_status ?? null;
    if (profileRequestStatus) existing.profile_requested_at = nowIso();
  ...
  const waitlist = {
    ...
    sms_opt_in: Boolean(smsOptIn || channelPreference === "NAVER_TALK_AND_SMS"),
    channel_preference: channelPreference,
    profile_request_status: profileRequestStatus,
    profile_result: null,
    profile_requested_at: profileRequestStatus ? nowIso() : null,
    profile_received_at: null,
    status: "WAITING",
    requested_at: nowIso(),
    restocked_at: null,
    notified_at: null,
...
export function updateWaitlistPhoneForUser({ talkUserId, productId = null, optionId = null, phoneNumber, profileResult = "SUCCESS" }) {
  const store = db();
  const candidates = store.waitlists.filter((item) => {
    if (item.talk_user_id !== talkUserId) return false;
    if (productId && item.product_id !== productId) return false;
    if (optionId && item.option_id !== optionId) return false;
    return item.status !== "CANCELLED";
  });

  for (const waitlist of candidates) {
    waitlist.phone_number = phoneNumber;
    waitlist.sms_opt_in = true;
    waitlist.channel_preference = "NAVER_TALK_AND_SMS";
    waitlist.profile_request_status = profileResult;
    waitlist.profile_result = profileResult;
    waitlist.profile_received_at = nowIso();
...
export function withdrawPhoneForUser(talkUserId) {
  const store = db();
  const affected = [];
  for (const waitlist of store.waitlists) {
    if (waitlist.talk_user_id !== talkUserId) continue;
    waitlist.phone_number = null;
    waitlist.sms_opt_in = false;
    waitlist.channel_preference = "NAVER_TALK_ONLY";
    waitlist.profile_request_status = "WITHDRAW";
    waitlist.profile_result = "WITHDRAW";
    waitlist.updated_at = nowIso();
    affected.push(waitlist);
  }
  for (const consent of store.consent_logs) {
    if (consent.talk_user_id === talkUserId && !consent.withdrawn_at) {
