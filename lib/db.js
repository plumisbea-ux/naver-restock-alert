...
export function getCustomerContext(talkUserId) {
  if (!talkUserId) return null;
  return db().customer_contexts?.[talkUserId] || null;
}

export function registerCustomerOpen({ talkUserId, productId = null, productNo = null, rawEvent = null }) {
  const store = db();
  if (!store.customer_contexts) store.customer_contexts = {};
  if (!talkUserId) return null;
  const previous = store.customer_contexts[talkUserId] || {};
  const context = {
    talk_user_id: talkUserId,
    product_id: productId || previous.product_id || null,
    product_no: productNo || previous.product_no || null,
    last_open_event: rawEvent || previous.last_open_event || null,
    opened_count: Number(previous.opened_count || 0) + 1,
    opened_at: nowIso(),
    updated_at: nowIso()
  };
  store.customer_contexts[talkUserId] = context;
  return context;
}

export function rememberMockUserPhone(talkUserId, phoneNumber) {
  const store = db();
  if (!store.mock_users) store.mock_users = {};
  if (!store.mock_users[talkUserId]) {
    store.mock_users[talkUserId] = { talk_user_id: talkUserId };
  }
  store.mock_users[talkUserId].mock_phone_number = phoneNumber;
  store.mock_users[talkUserId].updated_at = nowIso();
  return store.mock_users[talkUserId];
}
