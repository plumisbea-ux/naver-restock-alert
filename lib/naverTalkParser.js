...
export function extractProductFromOpenEvent(eventBody) {
  ...
  for (const candidate of candidates) {
    const product = getProductByProductNo(String(candidate).split("|")[0]);
    if (product) return product;
  }
  return null;
}

export function extractOptionFromOpenEvent(eventBody) {
  const options = eventBody?.options || {};
  const fromParts = String(options.from || "").split("|");
  const optionId = options.optionId || options.option_id || fromParts[1] || null;
  if (!optionId) return null;
  return db().product_options.find((option) => option.id === optionId || option.option_id === optionId) || null;
}
...
export function detectIntent(eventBody, session) {
  ...
  if (source.startsWith("CHANNEL:SMS")) {
    const parts = source.split(":");
    return { type: "CHANNEL_SMS", productId: parts[2] || session?.product_id, optionId: parts[3] || session?.option_id };
  }

  if (source.startsWith("CONSENT:KAKAO_RESTOCK_ALERT") || source.startsWith("CONSENT:SMS_RESTOCK_ALERT") || text.includes("동의")) {
    return { type: "SMS_CONSENT" };
  }
