...
function collectProductNoCandidatesFromUrl(value) {
  ...
  const productNoMatch = text.match(/[?&](?:productNo|product_no|productId|product_id|from)=([^&]+)/);
  ...
  try {
    ...
    pushCandidate(candidates, url.searchParams.get("productId"));
    pushCandidate(candidates, url.searchParams.get("product_id"));
  } catch {
    // not a full URL
  }
...
export function extractProductNoCandidatesFromOpenEvent(eventBody) {
  ...
  pushCandidate(candidates, eventBody?.from);
  pushCandidate(candidates, eventBody?.productNo);
  pushCandidate(candidates, eventBody?.product_no);
  pushCandidate(candidates, eventBody?.productId);
  pushCandidate(candidates, eventBody?.context?.productNo);
  pushCandidate(candidates, eventBody?.context?.product_no);
  pushCandidate(candidates, eventBody?.context?.from);

  for (const value of collectProductNoCandidatesFromUrl(options.referer)) pushCandidate(candidates, value);
  for (const value of collectProductNoCandidatesFromUrl(eventBody?.referer)) pushCandidate(candidates, value);
