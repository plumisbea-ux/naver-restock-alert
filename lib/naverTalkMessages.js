...
export function restockApplyPrompt(product, outOfStockOptions) {
  ...
  const text = [
    `현재 보고 계신 상품은 ${product.product_name}입니다.`,
    `품절 옵션: ${optionNames}`,
    "원하시는 옵션이 재입고되면 톡톡으로 알려드릴 수 있습니다.",
    "재입고 알림을 신청하시겠어요?"
  ].join("\n");

  return textMessage(text, [button("재입고 신청", `APPLY_RESTOCK:${product.id}`)]);
}

export function noStockPrompt(product) {
  const text = [
    `현재 ${product.product_name} 상품은 품절 상태입니다.`,
    "재입고 알림을 신청하시면 입고 후 톡톡으로 알려드립니다."
  ].join("\n");
...
export function optionSelectPrompt(product, outOfStockOptions) {
  const buttons = outOfStockOptions.slice(0, 10).map((option) => {
    const title = option.original_option_name.length > 10 ? option.original_option_name.slice(0, 10) : option.original_option_name;
...
export function channelSelectPrompt(product, option) {
  return textMessage(
    ...
    [
      ...
      button("카카오톡도", `CHANNEL:KAKAO:${product.id}:${option.id}`)
    ]
  );
}
...
export function kakaoConsentPrompt() {
  return textMessage(
    ...
    [
      button("동의하고 계속", "CONSENT:KAKAO_RESTOCK_ALERT"),
...
export function completePrompt({ product, option, channelPreference, waitCount }) {
  ...
  return textMessage(
    [
      ...
      `수신 채널: ${channelText}`,
      `현재 대기인원: ${waitCount}명`,
      "테스트용 mock 설정에 따라 약 10초 후 재입고 알림 로그가 생성됩니다."
    ].join("\n")
  );
}
